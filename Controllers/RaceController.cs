...
namespace RunGroopApp.Controllers
{
	public class RaceController : Controller
	{
		...
		public RaceController(IRaceRepository raceRepository, IPhotoService photoService, IHttpContextAccessor httpContext)
		{
        ...
        {
            var raceDetailes = await _raceRepository.GetByIdAsync(id);

            if (raceDetailes == null)
            {
                return View("Error");
            }

            if (!string.IsNullOrEmpty(raceDetailes.Image))
            {
                _ = _photoService.DeletePhotoasync(raceDetailes.Image);
            }

            _raceRepository.Delete(raceDetailes);
            return RedirectToAction("Index");
        }
    }
}
